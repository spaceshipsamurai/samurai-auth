...
/**
* Samurai auth plug-in for phpBB3*
...
function login_samurai($username, $password, $ip = '', $browser = '', $forwarded_for = '')
{
	global $db, $config;    
    ...
    try
    {
        $authUser = $authService->Authenticate($username, $password);
    }
    ...
    {        
        ...
        {
            return auth_admin($username, $password, $ip, $browser, $forwarded_for);
        }
	...
	if (!$row)
	{
        //no user account for this character, create a new one
        $row = array(
                'username'              => $username,
...
function auth_admin($username, $password, $ip = '', $browser = '', $forwarded_for = '') {
